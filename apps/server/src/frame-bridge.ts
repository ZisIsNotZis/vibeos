...
const source = String.raw`<script data-vibeos-frame-bridge>(function(){
  ...
  addEventListener("message",event=>{const message=event.data;if(!message||typeof message!=="object")return;
    if(message.type==="vibeos:init"&&typeof message.channel==="string"){channel=message.channel;document.documentElement.dataset.theme=message.theme;parent.postMessage({type:"vibeos:ready",channel},"*");return}
