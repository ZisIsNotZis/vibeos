...
export function readStructuredWorkerResult(path: string) {
  const value = JSON.parse(readFileSync(path, 'utf8')) as { status?: unknown; summary?: unknown; statePatches?: unknown; value?: unknown };
  ...
  const result: { status: 'ready'; summary: string; statePatches?: Array<{ appId: string; patch: Array<{ op: 'add' | 'replace' | 'remove'; path: string; value?: unknown }>; revision?: number }>; value?: unknown } = { status: 'ready', summary: value.summary };
  if (value.statePatches !== undefined) result.statePatches = value.statePatches as NonNullable<typeof result.statePatches>;
...
const workerResultSchema = {
  ...
  properties: {
    ...
    statePatches: { type: 'array', items: { type: 'object', additionalProperties: false, required: ['appId', 'patch'], properties: { appId: { type: 'string', minLength: 1 }, revision: { type: 'integer', minimum: 0 }, patch: { type: 'array', minItems: 1, items: { type: 'object', additionalProperties: false, required: ['op', 'path'], properties: { op: { type: 'string', enum: ['add', 'replace', 'remove'] }, path: { type: 'string', pattern: '^/' }, value: { type: ['object', 'array', 'string', 'number', 'boolean', 'null'] } } } } } } }
  }
};
...
const bridgeTypes = `type AppState={appId:string;revision:number;state:unknown}; interface VibeOSBridge { request(operation: unknown): Promise<unknown>; navigate(url: string, mode?: 'search_results' | 'destination'): Promise<unknown>; storage: { read(key: string): Promise<unknown>; write(key: string, value: unknown): Promise<unknown> }; state: { read(): Promise<AppState>; write(state: unknown, revision?: number): Promise<AppState>; subscribe(listener:(state:AppState)=>void):()=>void }; dispatch(intent: unknown): Promise<unknown>; ai: { command(command: string, options?: { scope?: 'app'|'descendants'|'world'|{appId:string}; context?: unknown; output?: 'result'|'modify'|'navigate'|'generate' }): Promise<unknown> }; }\ndeclare global { interface Window { vibeOS: VibeOSBridge } }\nexport {};\n`;
