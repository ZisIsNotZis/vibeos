...
function coherentOutcome(task: AgentTask) { return task.capability === 'app:identity' ? 'A recognizable, launchable app identity with a serious app-specific SVG icon.' : 'A polished, usable vertical slice whose primary workflow and every visible local control work; if the request is a creator, generator, editor, transformer, analyzer, or exporter, its current page must complete an input-to-action-to-observable-result loop.'; }
...
const bridgeClient = `// window.vibeOS is injected by the generated-surface host. It includes storage, browser-native local export through standard Blob + <a download>, navigation, dispatch, ai.command({scope,context,output}), and contextMenu.open(items, point). ai.command output=result returns value for the current page; output=navigate opens a concrete route; output=modify applies state/world changes; output=generate requests a concrete artifact/result. Do not access parent DOM, open runtime sockets, or use ambient host APIs.\n`;
