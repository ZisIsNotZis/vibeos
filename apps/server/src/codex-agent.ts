...
function buildPrompt(task: AgentTask, search: SearchLevel) {
  ...
  return [
    ...
    'AI commands receive a focused app-state snapshot plus a compact world index. For a state-only request, return statePatches that mutate the supplied state using RFC-6902-style add/replace/remove paths and its revision; do not edit source files for a state change. The runtime validates, persists, and broadcasts patches so visible apps refresh. Change source only when the request adds or repairs capabilities beyond the declared state. Do not access paths outside this staged job. Return only the structured result required by input/result.schema.json after the files are actually complete.'
  ].join('\n');
}
