...
| Missing surfaces can be requested lazily and cached by app/route. | Implemented/tested | `apps/server/src/runtime.ts`; `runtime.test.ts` cache and concurrent-generation tests. | Deterministic adapter tests do not prove provider-backed generation. |
...
| Resource access is separately policy-controlled. | Implemented/tested | `selectResourcePolicy`; policy tests. | Policy expresses intent; actual host/provider enforcement is not benchmarked. |
...
| Publication preserves mutable data and rolls back after validation failure. | Implemented/tested | `publishCandidate`; publication and rollback tests. | Failure models outside the tested callback remain unmeasured. |
...
The source snapshot used for the paper is VibeOS `0.1.0` at revision `90b23f277a52c1862a764a97a9ddd99c8630e4f4`; the working tree had pre-existing documentation edits during inspection. Browser E2E, visual screenshots, benchmark data, generated artifacts, and PDF compilation are not claimed as reproduced by this package.
