(function(){
  ...
  svg.addEventListener('pointerdown',e=>{const node=e.target.closest('.node-shape');const port=e.target.closest('.port');if(port&&node){const n=state.nodes.find(x=>x.id===node.dataset.id);link={from:n.id,start:worldPoint(e)};svg.setPointerCapture(e.pointerId);return;}if(node){select(node.dataset.id);const p=worldPoint(e),n=state.nodes.find(x=>x.id===node.dataset.id);drag={id:n.id,dx:p.x-n.x,dy:p.y-n.y};svg.setPointerCapture(e.pointerId);return;}if(e.target.closest('.edge'))return;pan={x:e.clientX,y:e.clientY,ox:state.view.x,oy:state.view.y};svg.setPointerCapture(e.pointerId);});
  ...
  svg.addEventListener('pointerup',e=>{if(drag){history.push(snapshot());if(history.length>45)history.shift();future=[];drag=null;scheduleSave('Shape moved');}else if(pan){pan=null;scheduleSave('View adjusted');}else if(link){const target=e.target.closest('.node-shape');if(target&&target.dataset.id!==link.from){const next=clone(state);if(!next.edges.some(x=>(x.from===link.from&&x.to===target.dataset.id)||(x.from===target.dataset.id&&x.to===link.from)))next.edges.push({id:'e-'+Date.now().toString(36),from:link.from,to:target.dataset.id});commit(next,'Connection added');notify('Shapes connected','success');}link=null;render();}});
  ...
  document.querySelectorAll('[data-align]').forEach(btn=>btn.addEventListener('click',()=>{const n=state.nodes.find(x=>x.id===selectedId);if(!n)return;const next=clone(state),targets=next.nodes.filter(x=>x.id!==selectedId);if(btn.dataset.align==='left'&&targets.length)n.x=Math.min(...targets.map(x=>x.x));if(btn.dataset.align==='right'&&targets.length)n.x=Math.max(...targets.map(x=>x.x+x.w))-n.w;if(btn.dataset.align==='top'&&targets.length)n.y=Math.min(...targets.map(x=>x.y));if(btn.dataset.align==='bottom'&&targets.length)n.y=Math.max(...targets.map(x=>x.y+x.h))-n.h;commit(next,'Shape aligned');}));
